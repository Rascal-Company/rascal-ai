...
export function usePostsQuery(user, t) {
  ...
  const mixpostQuery = useQuery({
    ...
    staleTime: 1000 * 60 * 5, // 5 minutes - Mixpost data doesn't change frequently
    refetchOnWindowFocus: false, // Don't refetch on window focus - expensive operation
    placeholderData: [],
  });
  ...
  return {
    ...
    setPosts: (newPosts) =>
      queryClient.setQueryData(["posts", orgId], newPosts),
    setReelsPosts: (newPosts) =>
      queryClient.setQueryData(["reels", user?.company_id], newPosts),
    setMixpostPosts: (newPosts) =>
      queryClient.setQueryData(["mixpost", orgId], newPosts),
