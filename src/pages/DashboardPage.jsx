...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      if (orgLoading || !orgId) {
        if (!orgLoading && !orgId) {
          setError(t('posts.ugcTab.userIdNotFound'))
          setLoading(false)
        }
        return
      }
      ...
      if (error) setError(t('posts.messages.fetchError'))
      setPosts(data || [])
      setLoading(false)
    }
    fetchPosts()
    ...
    <>
      ...
      <div className="p-4 sm:p-6 lg:p-10 max-w-[1600px] mx-auto min-h-screen">
        <div className="flex flex-col gap-2 mb-8 lg:mb-12">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">{t('dashboard.header.title')}</h1>
          <p className="text-gray-500 text-base sm:text-lg">{t('dashboard.header.subtitle')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-6">
          ...
          <div className="sm:col-span-2 lg:col-span-12 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
            <h2 className="text-xl font-semibold text-gray-800 tracking-tight">{t('dashboard.metrics.title')}</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
              {statsLoading ? (
                Array(6).fill(0).map((_, i) => (
                  <div key={i} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 h-24 animate-pulse"></div>
                ))
              ) : (
                [
                  ...dashboardStats,
                  ...
                  <div key={i} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-5 hover:shadow-md transition-shadow duration-200 flex flex-col justify-between min-h-[100px] sm:min-h-[110px]">
                    <div className="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1 sm:mb-2 truncate" title={stat.label}>
                      {stat.label}
                    </div>
                    <div className="flex items-baseline gap-2 flex-wrap">
                      ...
                      {stat.trend !== undefined && !stat.noTrend && (
                        <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded ${stat.trend > 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
          ...
          <div className="sm:col-span-2 lg:col-span-8 bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
              <h3 className="text-lg font-semibold text-gray-900">{t('dashboard.mixpost.title')}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 flex-1">
              {mixpostLoading ? (
                Array(4).fill(0).map((_, i) => <div key={i} className="bg-gray-50 rounded-lg h-28 animate-pulse"></div>)
              ) : (mixpostData ? (
                [
                  { label: t('dashboard.mixpost.metrics.fbEngagements'), value: (mixpostData.fbEngagements ?? 0).toLocaleString('fi-FI'), color: '#1877f2' },
                  { label: t('dashboard.mixpost.metrics.fbImpressions'), value: (mixpostData.fbImpressions ?? 0).toLocaleString('fi-FI'), color: '#1877f2' },
                  { label: t('dashboard.mixpost.metrics.igReach'), value: (mixpostData.igReach ?? 0).toLocaleString('fi-FI'), color: '#e4405f' },
                  { label: t('dashboard.mixpost.metrics.igFollowers'), value: (mixpostData.igFollowers ?? 0).toLocaleString('fi-FI'), color: '#e4405f' }
                ].map((metric, i) => (
                  <div key={i} className="p-5 rounded-2xl bg-gray-50/50 border border-gray-100 transition-all hover:bg-white hover:shadow-sm">
                    ...
                    <div className="h-1.5 w-full bg-gray-200 mt-4 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: '70%', background: metric.color }}></div>
                    </div>
                  </div>
                ))
              ) : (
                <div className="col-span-full text-center py-12 text-gray-400">{t('dashboard.mixpost.noData')}</div>
              ))}
            </div>
          </div>
          ...
          <div className="sm:col-span-2 lg:col-span-4 self-start bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-lg font-semibold text-gray-900">{t('dashboard.visitors.title')}</h3>
              {gaConnected && (
            ...
            {!gaConnected ? (
              <div className="flex flex-col items-center justify-center py-10 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm mb-4">
                  <svg className="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                </div>
                <p className="text-sm text-gray-400 max-w-[180px]">{t('dashboard.visitors.notConnected')}</p>
              </div>
            ) : gaLoading ? (
              ...
              <div className="flex flex-col">
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="p-3 rounded-xl bg-blue-50/30 border border-blue-50">
                    <div className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">{t('dashboard.visitors.total')}</div>
                    <div className="text-2xl font-bold text-gray-900">{gaVisitors.thisWeek.toLocaleString('fi-FI')}</div>
                  </div>
                  <div className="p-3 rounded-xl bg-orange-50/30 border border-orange-50">
                    <div className="text-[10px] font-bold text-orange-400 uppercase tracking-widest mb-1">{t('dashboard.visitors.today')}</div>
                    <div className="text-2xl font-bold text-gray-900">{gaVisitors.today.toLocaleString('fi-FI')}</div>
                  </div>
                </div>
                ...
                <div className="h-[140px] w-full -mx-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={gaVisitorsData}>
                      ...
                      <Area type="monotone" dataKey="visitors" stroke="#ff6600" fillOpacity={1} fill="url(#colorVisitors)" strokeWidth={3} />
                      <Tooltip
                        contentStyle={{ fontSize: '12px', borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }}
          ...
          <div className="sm:col-span-2 lg:col-span-8 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
              <h3 className="text-lg font-semibold text-gray-900">{t('dashboard.upcoming.title')}</h3>
              <button className="text-xs font-semibold text-blue-600 hover:text-blue-700">{t('dashboard.upcoming.today')}</button>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[500px]">
                <thead className="bg-gray-50/50">
                  <tr>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('dashboard.upcoming.headers.media')}</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('dashboard.upcoming.headers.caption')}</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('dashboard.upcoming.headers.channels')}</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('dashboard.upcoming.headers.date')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {scheduleLoading ? (
                    Array(4).fill(0).map((_, i) => <tr key={i}><td colSpan={4} className="px-6 py-4 h-16 animate-pulse bg-gray-50/20"></td></tr>)
                  ) : (upcomingPosts.length > 0 ? (
                    upcomingPosts.slice(0, 5).map((row) => (
                      <tr key={row.id} onClick={() => handleScheduledPostClick(row)} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                        <td className="px-6 py-4">
                          {(() => {
                            const url = Array.isArray(row.media_urls) ? row.media_urls[0] : (typeof row.media_urls === 'string' ? row.media_urls : null)
                            return url ? (
                              <img src={url} className="w-12 h-12 rounded-lg object-cover border border-gray-100 group-hover:scale-105 transition-transform" onClick={(e) => { e.stopPropagation(); setImageModalUrl(url); }} />
                            ) : (
                              <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center text-[10px] text-gray-400 font-bold uppercase text-center leading-tight">No image</div>
                            )
                          })()}
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm font-medium text-gray-700 line-clamp-2 max-w-[200px] leading-relaxed">{row.caption || '--'}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1.5">
                            {row.accounts?.map((acc, idx) => (
                              <span key={idx} className="bg-blue-50 text-blue-600 text-[9px] font-bold px-1.5 py-0.5 rounded-full border border-blue-100/50">
                                {acc.name || acc.provider}
                              </span>
                            ))}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-medium">{formatUpcomingDate(row.publish_date)}</td>
                      </tr>
                    ))
                  ) : (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-400">{t('dashboard.upcoming.empty')}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
          ...
          <div className="sm:col-span-2 lg:col-span-4 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-gray-100 bg-white">
              <h3 className="text-lg font-semibold text-gray-900">{t('dashboard.campaigns.title')}</h3>
            </div>
            <div className="flex-1 overflow-y-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50/50">
                  <tr>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t('dashboard.campaigns.headers.campaign')}</th>
                    <th className="px-6 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-right">{t('dashboard.campaigns.headers.successRate')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {campaignMetrics.length > 0 ? (
                    campaignMetrics.slice(0, 8).map(row => (
                      <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 text-sm font-semibold text-gray-700">{row.name}</td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <div className="w-12 h-1 bg-gray-100 rounded-full overflow-hidden hidden xl:block">
                              <div className="h-full bg-green-500 rounded-full" style={{ width: `${row.successRate}%` }}></div>
                            </div>
                            <span className="text-[10px] font-bold text-green-600 bg-green-50 px-1.5 py-0.5 rounded-md">{row.successRate}%</span>
                          </div>
                        </td>
                      </tr>
                    ))
                  ) : (
                    <tr><td colSpan={2} className="px-6 py-12 text-center text-gray-400">{t('dashboard.campaigns.noCampaigns')}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
