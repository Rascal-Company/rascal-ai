...
- **GH-3 (MUST)** Bump version after committing using `standard-version`:
  - `feat:` commits → `npm run release:minor` (MINOR bump)
  - `fix:`, `perf:` commits → `npm run release:patch` (PATCH bump)
  - `BREAKING CHANGE` → `npm run release:major` (MAJOR bump)
  - `docs:`, `style:`, `refactor:`, `test:`, `build:`, `ci:`, `chore:` → `npm run release:patch` (PATCH bump)
...
[optional body]
[optional footer(s)]
- commit MUST contain the following structural elements to communicate intent:
...
After committing, MUST bump version and update CHANGELOG:
1. Determine the bump type from the commit type:
   - feat: → npm run release:minor
   - fix:, perf: → npm run release:patch
   - BREAKING CHANGE → npm run release:major
   - All other types (docs, style, refactor, test, build, ci, chore) → npm run release:patch
2. Run the appropriate release command. This will:
   - Bump version in package.json
   - Update CHANGELOG.md
   - Create a version commit and git tag
3. Push the commit AND tags: git push && git push --tags
```
