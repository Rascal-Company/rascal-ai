...
export default function CarouselsTab({
  ...
  // Use TanStack Query for carousel data - replaces useState + useEffect
  const { data: carouselsData = [], refetch: refetchCarousels } =
    useCarouselPosts();
  ...
  // Data fetching now handled by TanStack Query hook above
  ...
  const handleSave = async () => {
    try {
      ...
      if (updates.length === 0) {
        setSaveMessage({
          type: "info",
          text: t("posts.carouselsTab.messages.noChanges"),
        });
        return;
      }
      ...
      for (const update of updates) {
        ...
        if (error) {
          if (error.message && error.message.includes("approved")) {
            ...
            throw new Error(
              t("posts.carouselsTab.messages.segmentUpdateFailed", {
                id: update.id,
              }),
            );
          }
      ...
      setSaveMessage({
        type: "success",
        text: t("posts.carouselsTab.messages.saved", { count: updates.length }),
      });

      // Refetch data using TanStack Query
      ...
      setSaveMessage({
        type: "error",
        text: error.message || t("posts.carouselsTab.messages.saveFailed"),
      });
    } finally {
      setSaving(false);
    }
  ...
  const activeCarousel = selectedCarouselId
    ? carouselsData.find((c) => c.content.recordId === selectedCarouselId)
    : null;
  ...
  return (
    <div className="space-y-12 pb-24">
          ...
          {selectedCarouselId ? (
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">
                {t("posts.carouselsTab.editor.backToList")}
              </span>
            </button>
          ) : null}
          <h2 className="text-3xl font-black text-gray-900 tracking-tight">
            {selectedCarouselId
              ? activeCarousel?.content.idea ||
                t("posts.carouselsTab.editor.editingTitle")
              : t("posts.carouselsTab.title")}
          </h2>
          {carouselsData.length > 0 && !selectedCarouselId && (
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest pl-2">
                {t("posts.carouselsTab.editor.workingPreview", {
      ...
      {saveMessage && (
        <div
          className={`p-6 rounded-[32px] border-2 animate-in slide-in-from-top-4 duration-500 flex items-center justify-between shadow-xl shadow-gray-200/20 ${
        ...
        >
          <div className="flex items-center gap-4">
            <div
              className={`w-10 h-10 rounded-2xl flex items-center justify-center ${saveMessage.type === "success" ? "bg-emerald-100" : "bg-red-100"}`}
            >
          ...
          <button
            ...
            className="text-[10px] font-black uppercase tracking-widest hover:opacity-100 opacity-40"
          >
            {t("common.close")}
          </button>
        </div>
      ...
      {carouselsData.length === 0 ? (
        <div className="py-32 flex flex-col items-center justify-center text-center bg-white rounded-[48px] border border-gray-100 shadow-[0_32px_96px_-16px_rgba(0,0,0,0.05)] group">
          <div className="relative mb-8">
            ...
            <div className="relative w-24 h-24 bg-white rounded-[32px] shadow-xl shadow-gray-100 flex items-center justify-center text-5xl group-hover:scale-110 transition-all duration-500 group-hover:rotate-6">
              🎠
            </div>
          </div>
          ...
          <p className="text-gray-400 text-sm font-bold uppercase tracking-widest max-w-xs">
            {t("posts.carouselsTab.empty.text") ||
              "Ei uusia karuselleja työn alla."}
          </p>
        </div>
      ) : !selectedCarouselId ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 animate-in fade-in duration-700">
          {carouselsData.map((item, idx) => {
            const dateStr = item.content.created
              ? new Date(item.content.created).toLocaleDateString(
                  ...
                  <div className="p-6 bg-gray-50/50 rounded-3xl border border-gray-100/50 min-h-[80px]">
                    <p className="text-[11px] font-bold text-gray-400 italic leading-relaxed line-clamp-2">
                      {item.content.caption ||
                        t("posts.carouselsTab.list.noDescription")}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex -space-x-2">
                      {item.content.segments?.slice(0, 5).map((s, i) => (
                        <div
                          key={i}
                          className="inline-block h-8 w-8 rounded-xl ring-4 ring-white bg-gray-100 overflow-hidden flex-shrink-0 z-[1]"
                        >
                          {s.media_urls?.[0] ? (
                            <img
                              src={s.media_urls[0]}
                            ...
                            <div className="h-full w-full flex items-center justify-center text-[9px] font-black text-gray-300">
                              {i + 1}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">
                        {t("posts.carouselsTab.list.pagesLabel")}
                      </p>
                      <p className="text-sm font-black text-gray-900">
                        {item.content.segments?.length || 0}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-8 order-2 lg:order-1">
            <div className="p-10 bg-white rounded-[48px] border border-gray-100 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] space-y-8 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8">
                <span className="text-[40px] opacity-[0.03] font-black select-none pointer-events-none">
                  Focus
                </span>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    {t("posts.carouselsTab.editor.themeLabel")}
                  </label>
                </div>
                <div className="text-lg font-bold text-gray-900 leading-snug">
                  {activeCarousel.content.idea ||
                    t("posts.carouselsTab.editor.untitled")}
                </div>
              </div>

              <div className="p-8 bg-gray-50/50 rounded-[32px] border border-gray-100/50">
                <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-4">
                  {t("posts.carouselsTab.editor.contextLabel")}
                </label>
                <p className="text-sm font-medium text-gray-600 leading-relaxed italic">
                  "
                  {activeCarousel.content.caption ||
                    t("posts.carouselsTab.editor.noDescription")}
                  "
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-4">
                <div className="space-y-1">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block">
                    {t("posts.carouselsTab.editor.statusLabel")}
                  </span>
                  <span className="px-3 py-1 bg-white border border-gray-100 rounded-full text-[10px] font-bold text-gray-900 shadow-sm">
                    {activeCarousel.content.status}
                  </span>
                </div>
                <div className="space-y-1 text-right">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block">
                    {t("posts.carouselsTab.editor.pagesLabel")}
                  </span>
                  <span className="text-sm font-black text-gray-900">
                    {activeCarousel.content.segments.length}
                  </span>
                </div>
              </div>
            ...
            <div className="relative space-y-4">
              ...
              {activeCarousel.content.segments.map((segment, segIndex) => {
                ...
                return (
                  <div
                    key={segmentId}
                    className="flex gap-4 group/segment items-start"
                  >
                    {/* Index Indicator */}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0 flex items-center py-1">
                          <textarea
                            ...
                            placeholder={t(
                              "posts.carouselsTab.editor.segmentPlaceholder",
                            )}
                            rows={1}
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          >
                            {currentApproved
                              ? t("posts.carouselsTab.editor.ok")
                              : t("posts.carouselsTab.editor.editing")}
                          </div>
